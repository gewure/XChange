# Arbitrage Bot Integration Guide: Uniswap V3 Adapter
...
// A. Network Selection
// Use UniswapNetwork enum for presets (Mainnet, Polygon, Arbitrum, Optimism)
// For Testnets (Sepolia), use ETHEREUM but override the RPC and Address.
spec.setNetwork(UniswapNetwork.ETHEREUM); 
...
spec.setExchangeSpecificParametersItem(UniswapExchangeSpecification.FLASHBOTS_RELAY_URI, "https://relay.flashbots.net");
spec.setExchangeSpecificParametersItem(UniswapExchangeSpecification.FLASHBOTS_RELAY_SIGNING_KEY, "0x..."); // Different random key

// E. Arbitrage Specifics
spec.setExchangeSpecificParametersItem(UniswapExchangeSpecification.SLIPPAGE_TOLERANCE, 0.005); // 0.5% Slippage
...
When you find an opportunity (e.g., Uniswap Price < Binance Price), execute immediately.
```java
// Prepare Order
...
MarketOrder buyOrder = new MarketOrder(
    ...
    new UniswapInstrument(new Currency("WETH"), new Currency("USDC"), "0xPoolAddress...", 3000) // 0.3% Fee Tier
);
...
2.  **Get Tokens**: Swap Sepolia ETH for Uniswap tokens on the official Uniswap Interface (connected to Sepolia).
3.  **Config**:
    ```java
    spec.setNetwork(UniswapNetwork.ETHEREUM); // Use ETH base config
    ...
    // but standard Uniswap V3 addresses are usually the same across testnets.
